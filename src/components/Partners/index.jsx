...
const Partners = ({ className }) => {
  ...
  const [partnersChunks, setPartnersChunks] = useState([]);
  ...
  const partnerChunksData = useMemo(() => {
    const result = [];
  ...
  // Directly update the state when chunked data changes
  useEffect(() => {
    setPartnersChunks(partnerChunksData);
  }, [partnerChunksData]);
